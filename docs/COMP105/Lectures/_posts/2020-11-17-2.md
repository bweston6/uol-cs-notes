...
```haskell
> takeWhile (\#92; x -> length x <= 2) ["ab","cd","efg"]
> ["ab", "cd"]
```
...
```haskell
> dropWhile (\#92; x -> x < 10 && x > 0) [1,2,3,10,4,5]
> [10,4,5]
```
...
split_words string =
	let
		...
		up_to_space = dropWhile (\#92;= ' ') string
...
> zipWith (\#92; x y -> if x then y else -y) [True, False, True] [1,2,3]
> [1,-2,-3]
```
...
interleave str1 str2 = 
	let
		zipped = zipWith (\#92; x y -> [x,y]) str1 str2
	in
		concat zipped
...
## Exercises
1. ```haskell
	getNumber string = takeWhile (\#92; x -> elem x ['0'..'9']) string
	```
