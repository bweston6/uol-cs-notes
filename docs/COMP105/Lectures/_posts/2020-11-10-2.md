...
The `\)</pre>{% endraw %}` operator is exactly the same as evaluate. `{% raw %}<pre>\(` is infix.

The `\)</pre>{% endraw %}` operator has the lowest precedence of any operator. As a result it can be used in place of brackets:
