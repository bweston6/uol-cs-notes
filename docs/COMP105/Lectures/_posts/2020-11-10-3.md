...
The `\` is supposed to resemble an lambda {% raw %}<![CDATA[\(\lambda\)]]>{% endraw %}.

* Anonymous function are sometimes called {% raw %}<![CDATA[\(\lambda\)]]>{% endraw %}-functions in recognition of lambda calculus.
