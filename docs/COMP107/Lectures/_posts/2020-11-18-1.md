...
#### Cardinality 
Some attributes may have more than one value. If this is the case then you can say that a particular value has a cardinality {% raw %}<![CDATA[\(>0\)]]>{% endraw %}.
...
* The cardinality is represented on the connecting lines (an {% raw %}<![CDATA[\(N\)]]>{% endraw %} represents the many side.
