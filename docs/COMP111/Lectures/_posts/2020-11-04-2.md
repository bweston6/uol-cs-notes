...
Instead of {% raw %}<![CDATA[\(\text{MiniMaxV}(s)\)]]>{% endraw %} we compute {% raw %}<![CDATA[\(\text{CutOffV}(s)\)]]>{% endraw %}.

Assume that we con compute a function {% raw %}<![CDATA[\(\text{Evaluation}(s)\)]]>{% endraw %} which gives us a utility value for any state {% raw %}<![CDATA[\(s\)]]>{% endraw %} which we do not want explored (every cutoff state). 

Then define {% raw %}<![CDATA[\(\text{CutOffV}(s)\)]]>{% endraw %} recursively:
...
* Minimax algorithm (with {% raw %}<![CDATA[\(\alpha - \beta\)]]>{% endraw %} pruning) fundamental for game playing.
