...
* S{% raw %}<![CDATA[\(_\textrm{start}\)]]>{% endraw %} for the starting point.
* S{% raw %}<![CDATA[\(_\textrm{goal}\)]]>{% endraw %} for the goal states.
* A set of A actions that can be performed that will take the agent from one state in S to another state in S.
* There can sometimes be a cost function that states how expensive an action is. If all actions are equally expensive, we set {% raw %}<![CDATA[\(\textrm{cost}(a)=1\)]]>{% endraw %}

A solution to the search problem is a sequence of actions such that when performed in the start state S{% raw %}<![CDATA[\(_\textrm{start}\)]]>{% endraw %}, that agents reaches a goal state in S{% raw %}<![CDATA[\(_\textrm{goal}\)]]>{% endraw %}. A solution is *optimal* if there is no less expensive solution.
