...
* S\#92;(_\#92;textrm{start}\#92;) for the starting point.
* S\#92;(_\#92;textrm{goal}\#92;) for the goal states.
* A set of A actions that can be performed that will take the agent from one state in S to another state in S.
* There can sometimes be a cost function that states how expensive an action is. If all actions are equally expensive, we set \#92;(\#92;textrm{cost}(a)=1\#92;)

A solution to the search problem is a sequence of actions such that when performed in the start state S\#92;(_\#92;textrm{start}\#92;), that agents reaches a goal state in S\#92;(_\#92;textrm{goal}\#92;). A solution is *optimal* if there is no less expensive solution.
