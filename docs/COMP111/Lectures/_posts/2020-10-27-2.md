...
* We have a **heuristic function** {% raw %}<pre>\[ h:\text{States}\rightarrow\text{real numbers}\]</pre>{% endraw %} which estimates the cost of going form that state to the goal. {% raw %}<pre>\(h\)</pre>{% endraw %} can be any function by {% raw %}<pre>\(h(s) = 0\)</pre>{% endraw %} if {% raw %}<pre>\(s\)</pre>{% endraw %} is a goal.
