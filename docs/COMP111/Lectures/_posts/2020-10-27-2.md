...
* We have a **heuristic function** {% raw %}\]{% endraw %} h:\text{States}\rightarrow\text{real numbers}\] which estimates the cost of going form that state to the goal. {% raw %}\(h\){% endraw %} can be any function by {% raw %}\(h(s) = 0\){% endraw %} if {% raw %}\(s\){% endraw %} is a goal.
