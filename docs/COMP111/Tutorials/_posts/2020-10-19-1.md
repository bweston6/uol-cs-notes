...
1. For a path {% raw %}<![CDATA[\(a_0 \rightarrow a_1 \rightarrow a_2 \rightarrow a_3\)]]>{% endraw %}, the length of the path is 3 as that is the number of actions that need to be performed.
...
1. 
	* This problem is very large so I will describe the result
		* The set of {% raw %}<![CDATA[\(S\)]]>{% endraw %} states are all 16 or {% raw %}<![CDATA[\(4^4\)]]>{% endraw %} states that exist as a subset of the set {% raw %}<![CDATA[\(N = \{ 1,2,3,4 \}\)]]>{% endraw %}
		* The start state {% raw %}<![CDATA[\(S_{start}\)]]>{% endraw %} is an empty subest {% raw %}<![CDATA[\(\{ \phi \}\)]]>{% endraw %} with a sum of zero.
		* The set of goal states {% raw %}<![CDATA[\(S_{goal}\)]]>{% endraw %} are all the subsets which add to 6
		* The set of actions {% raw %}<![CDATA[\(A\)]]>{% endraw %} are any addition or subtraction of a number that is in the set.
		* The cost function for each action should just be one such that the shortest route from {% raw %}<![CDATA[\(S_{start}\)]]>{% endraw %} is achieved.
